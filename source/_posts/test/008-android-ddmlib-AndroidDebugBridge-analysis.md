---
title: Android ddmlib AndroidDebugBridge源代码分析
date: 2017-07-16 21:38:32
tags:
categories: "测试开发"
---
...
* 在使用 _createBridge_ 方法之前必须要先调用一次 _init_ 方法；

* _init_ 方法中的方法参数 _clientSupport_ 的意义就是如果连接的设备需要监测或者与应用交互，则应设置为true；否则，false；

* _createBridge_ 两种方法的区别，不带参数的方法，依赖adb已启动；带方法参数要指定adb的路径和是否强制创建adb连接，唯一部重新创建adb连接的条件是：上一次的adb连接存在，且方法参数中的adb路径和上一次一致，并且第二个参数forceCreate不为true；

* 其实创建adb连接桥主要还是调用adb命令的start-server，以及停止时是调用的stop-server命令;

* 可以通过指定环境变量来改变默认的adb连接端口(5037)，也就是环境变量中添加设置 _ANDROID_ADB_SERVER_PORT_ 的值；

* 可以在此类中添加adb连接桥的状态变化监听器，客户端的状态变化监听器以及设备的状态变化监听器；
...
public final class AndroidDebugBridge {
     ...
     *
     * AndroidDebugBridge支持的最小adb版本为1.0.20
     */
     ...
     * 初始化ddm库
     *
     * init必须在调用createBridge(String, boolean)之前执行一次
     *
     */
